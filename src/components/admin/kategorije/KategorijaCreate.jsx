...
const KategorijaCreate = ({ kategorijaid }) => {
    ...
    useEffect(() => {
        axiosClient.get('/admin/get-sve-kategorije').then((res) => {
            setSveKategorije(res.data)
        }).catch((error) => {
            console.error(error)
        });
        if (kategorijaid) {
            axiosClient.get(`/admin/get-single-kategorija/${kategorijaid}`)
                .then((res) => {
    ...
    const optionsKategorije = sveKategorije.map(kategorija => ({
        value: kategorija.kategorijaid, label: kategorija.naziv_kategorije
    }));
    ...
    const handleSubmit = (event) => {
        ...
        if (formData.kategorijaid) {
            axiosClient.put('/admin/update-kategorija', formData)
                .then((res) => {
                    console.log(res);
            ...
            axiosClient.post('/admin/create-kategorija', formData)
    ...
    return (
        <>
            <Stack component="form" direction="column" spacing={2} alignItems="center" sx={{ width: 300 }} marginX={"auto"}>
                <TextField name="naziv_kategorije" label="Naziv kategorije" variant="outlined" value={formData.naziv_kategorije} onChange={handleChange} />
                <TextField name="kategorija_slug" label="Slug" variant="outlined" value={formData.kategorija_slug} onChange={handleChange} />
                <FormControl fullWidth>
                    ...
                    >
                        {sveKategorije.map((kat) =>
                            <MenuItem key={kat.kategorijaid} value={kat.kategorijaid}>{kat.naziv_kategorije}</MenuItem>
                        )}
                ...
                <FormControl>
                    <ColorPicker name="kategorija_boja" value={formData.kategorija_boja} onChange={handleColorChange} />
                </FormControl>
                <FormControl>
                    Preview: <a
