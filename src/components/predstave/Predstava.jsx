...
const Predstava = ({ data, premijere, sidePosts }) => {
    const recenzije = data.tekstovi?.filter((tekst) => tekst.kategorija.kategorijaid == 4);
    const povezaniTekstovi = data.tekstovi?.filter((tekst) => tekst.kategorija.kategorijaid != 4);
                                ...
                                <div className="single-blog-wrapper">
                                    {!showAllComments &&
                                        <div className="grid-wrapper predstava-single-section-wrapper">
                                            <div className="row">
                                                ...
                                                {description?.length > 0 && <div dangerouslySetInnerHTML={{ __html: description }}></div>}

                                                {description?.length > 1000 &&
                                                    <div className="m-t-xs-20">
                                                        <button className="btn btn-primary btn-small" onClick={showFullDescriptionHandler}>
                                                ...
                                                <div className="row">
                                                    ...
                                                    {recenzije.map((tekst) =>
                                                        <div key={tekst.tekstid}>
                                                            <PredstavaRecenzija data={tekst} key={tekst.slug} />
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    <div className="grid-wrapper predstava-single-section-wrapper">
                                        <div className="row">
                                            ...
                                            <SectionTitle title={`Komentari (${data.komentari?.length})`} />
                                            {showAllComments ?
                                                data.komentari?.map((komentar) =>
                                                    <>
                                                        <div className="predstava-komentar-wrapper m-b-xs-20" key={komentar.komentarid}>
                                                            <div className="predstava-komentar-info">
                                                                <strong>{komentar.korisnik.korisnicko_ime}</strong> | <span className="text-muted">{moment(komentar.created_at).fromNow()}</span>
                                                            </div>
                                                            <div className="predstava-komentar-tekst">{komentar.tekst_komentara}</div>
                                                        </div>
                                                    </>
                                                ) :

                                                data.komentari?.slice(0, 3).map((komentar) =>
                                                    <>
                                                        <div className="predstava-komentar-wrapper m-b-xs-20" key={komentar.komentarid}>
                                                            <div className="predstava-komentar-info">
                                                                <strong>{komentar.korisnik.korisnicko_ime}</strong> | <span className="text-muted">{moment(komentar.created_at).fromNow()}</span>
                                            ...
                                            {data.komentari?.length > 3 &&
                                                <button onClick={toggleCommentsSection}>
                                                    {showAllComments ? "Nazad" : "Prikazi sve komentare"}
                                                </button>
                                            }
                                        </div>
                                    </div>
                                        ...
                                        <div className="row">
                                            {!showAllComments &&
                                                <>
                                                    ...
                                                    {izvodjenja?.length > 0 ?
                                                        izvodjenja?.slice(0, 9).map((item) =>
                                                            <Izvodjenje izvodjenjeData={item} showPredstava={false} showPozoriste={true} key={item.seigraid} />
                                                        )
                                                        :
                                                        <p>Predstava trenutno nije na repertoaru.</p>
                                                    }
                                                </>
                                            }
                                        </div>
                                    ...
                                    <div className="grid-wrapper predstava-single-section-wrapper">
                                        <div className="row">
                                            {povezaniTekstovi?.length > 0 && !showAllComments &&
                                                <>
                                                    <SectionTitle title="Povezani tekstovi" />
                                                    {povezaniTekstovi.map((tekst) =>
                                                        <PostLayoutTwo data={tekst} postSizeMd={false} key={tekst.slug} />
