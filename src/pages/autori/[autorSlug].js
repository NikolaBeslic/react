...
const PostAuthor = () => {
    ...
    const fetchSingleAutor = async (autorSlug, page) => {
        showLoading();
        axiosClient.get(`/get-single-autor/${router.query.autorSlug}?page=${page}`)
            .then((res) => {
                console.log(res.data);
                setAutor(res.data);
                setAutorPosts(prevAutorPosts => [...prevAutorPosts, ...res.data.tekstovi]);
                hideLoading();
                            ...
                            <div className="author-details-block">
                                ...
                                <div className="media post-block post-block__mid m-b-xs-0">
                                    ...
                                    <div className="media-body">
                                        <h2 className="h4 m-b-xs-15">{autor.ime_autora}</h2>
                                        <p className="hover-line autor-pozicija"><i className="fa-regular fa-circle-user"></i> {autor.pozicija}</p>
                                        <div className="mid"><p dangerouslySetInnerHTML={{ __html: autor.biografija }}></p></div>
                                        <div className="post-metas">
                                            <ul className="list-inline">
                                                <li><i className="fa-light fa-location-dot"></i>{autor.grad?.naziv_grada}</li>
                                                <li><a href="#"><i className="fal fa-user-edit" />Ukupno tekstova ({autor.tekstovi?.length})</a></li>
                                            </ul>
                                        </div>
                                        <div className="author-social-share">
                                            <ul className="social-share social-share__with-bg">
                                                ovde su bile ikonice za drustvene mreze
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="row">
                        <div className="col-lg-8">
                            ...
                            <div className="axil-content">
                                <h2 className="h3 m-b-xs-40">Teksovi autorke</h2>
                                {autorPosts.map((data) => (
                                    <PostLayoutTwo data={data} postSizeMd={true} key={data.slug} />
                                ))}
                                {!isLoading && <button className="btn btn-primary btn-small" onClick={() => setCurrentPage(currentPage + 1)}>Ucitaj jos</button>}
                            </div>
                        </div>
