...
const RepertoariNewPage = () => {
    ...
    useEffect(() => {
        ...
        axiosClient.get('/get-predstave-with-texts')
            .then((res) => {
                console.log(res.data);
    ...
    useEffect(() => {
        ...
        if (datumDo) {
            filteredEvents = dbEvents.filter((dbe) =>
                new moment(dbe.datum).isSameOrAfter(mDatumOd) && new moment(dbe.datum).isBefore(mDatumDo));
        }
        else {
            filteredEvents = dbEvents.filter((dbe) => new moment(dbe.datum).isSame(mDatumOd));
        }
        ...
        if (selectedGradovi.length > 0) {
            ...
            filteredEvents = filteredEvents.filter(igranje =>
    ...
    return <>
        ...
        <div className="axil-about-us section-gap-top p-b-xs-20">
            <div className="container">
                <h1>Repertoari</h1>
                <p> {datumOd?.toDateString()} - {datumDo?.toDateString()}</p>
                <div className="row">
                    <div className="col-lg-6">
                        ...
                        <p>
                            <button onClick={getWeekEvents}>Ove nedelje</button>
                        </p>
                        <p>
                            <button onClick={getMonthEvents}>Ovog meseca</button>
                        </p>
                        <div>
                    ...
                    <div className="col-lg-6">
                        <div className="repertoari-wrapper">
                            {displayEvents.map((e, index, arr) => {
                                ...
                                let printDate = '';
                                if (prevEvent?.datum != e.datum) printDate = e.datum;
                                return (
                                    <>
                                        <p>{printDate}</p>
                                        <Izvodjenje izvodjenjeData={e} showPozoriste={true} showPredstava={true} />
                                    </>
                                )
                            })}
                        </div>
                    ...
                    <div className="col-lg-12">
                        ...
                        {predstave.map((pred) =>
                            <PredstaveLayout data={pred} pClass="" showPozoriste={false} key={pred.predstavaid} />
                        )}
                    </div>
                </div>
            </div>
